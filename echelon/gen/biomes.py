...
def fill_command_outpost(
    ...
    world.set_box_solid(
        cx - bw // 2 + 2, cy - bh // 2 + 2, 0, cx + bw // 2 - 2, cy + bh // 2 - 2, bz - 1, VoxelWorld.SOLID
    )
    # Entrance (AIR)
...
def _build_stairs(
    world: VoxelWorld, x0: int, y0: int, z0: int, x1: int, y1: int, z1: int, width: int
) -> None:
    ...
    for i in range(steps + 1):
        ...
        pz = z0 + (z1 - z0) * t
        _ix, _iy, iz = int(px), int(py), int(pz)
        if iz >= 0:
            world.set_box_solid(
                int(px - half_w),
                int(py - half_w),
...
def _carve_worm(world: VoxelWorld, x0: int, y0: int, z0: int, x1: int, y1: int, z1: int, radius: int) -> None:
    ...
    for i in range(steps + 1):
        ...
        world.set_box_solid(
            ix - radius, iy - radius, iz - radius, ix + radius + 1, iy + radius + 1, iz + radius + 1, False
        )
...
def fill_orbital_relay(
    ...
    for angle in range(0, 360, 10):
        ...
        world.set_box_solid(
            px - 1, py - 1, 0, px + 2, py + 2, _safe_integers(rng, 8, 15), VoxelWorld.REINFORCED
        )
