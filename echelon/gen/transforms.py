...
def apply_transform_solids(solids_zyx: np.ndarray, transform: TransformType) -> np.ndarray:
    if transform == "identity":
        return solids_zyx
    if transform == "flip_x":
        return np.ascontiguousarray(np.flip(solids_zyx, axis=2))
    if transform == "flip_y":
        return np.ascontiguousarray(np.flip(solids_zyx, axis=1))
    if transform == "flip_xy":
        return np.ascontiguousarray(np.flip(solids_zyx, axis=(1, 2)))
