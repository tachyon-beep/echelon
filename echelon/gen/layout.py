...
from .biomes import CATALOG as BIOME_CATALOG
from .biomes import get_biome_brush
...
def generate_layout(config: WorldConfig, rng: np.random.Generator) -> VoxelWorld:
    ...
    jitter_x = int(sx * 0.1)
    jitter_y = int(sy * 0.1)
    split_x = int(mid_x + rng.integers(-jitter_x, jitter_x + 1))
    split_y = int(mid_y + rng.integers(-jitter_y, jitter_y + 1))
    ...
    assignment = dict(zip(q_names, chosen_biomes, strict=False))
