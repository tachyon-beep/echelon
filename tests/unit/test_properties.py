...
class SimStateMachine(RuleBasedStateMachine):
    ...
    def step_mech(self, mech_idx, action):
        ...
        # 2. Stability bounded
        assert 0.0 <= mech_post.stability <= mech_post.max_stability + 1e-6, f"Stability OOB: {mech_post.stability}"
        ...
        if mech_post.shutdown and mech_post.fallen_time <= 0.0:
            if abs(action[ActionIndex.YAW_RATE]) > 0.1:
